...
public class Login_modelo {
    ...
    static boolean login(String username, String password) {
        ...
        // Si el login es correcto, mostrar un mensaje de login correcto y abrir la página Main
        if (login) {
    ...
    private static boolean comprobarUsuario(String username) {
        // Inicializar la variable usuarioExistente a false
        ...
        if (conexionBD == null) {
            ...
            try (Statement consulta = conexionBD.createStatement()) {
                ...
                String sql = "SELECT * FROM Jugador WHERE username = '" + username + "'";
                consulta.executeUpdate(sql);
                // Si la consulta devuelve un resultado, el usuario ya existe
                if (consulta.getResultSet().next()) {
                    usuarioExistente = true;
                }
                // Si la consulta no devuelve un resultado, el usuario no existe
            } catch (SQLException e1) {
                System.out.println("Error al ejecutar la consulta");
            }
            // Cerrar la conexión con la BD
        ...
        // Devolver el valor de la variable usuarioExistente
